...
module {
  ...
  private func validateAccountProof(
    ...
    if (Blob.toArray(account).size() != 20) { // Ethereum address is 20 bytes
      return false;
    };
    
    if (Blob.toArray(stateRoot).size() != 32) { // State root is 32 bytes
      return false;
    };
  ...
  public func convertJsonWitnessToBlob(
    ...
    for (proof in accountProof.vals()) {
      switch (hexTextToBlob(proof)) {
        ...
        case (#err(msg)) { return #err("Invalid account proof element: " # msg) };
      };
    };
    ...
    for (proof in storageProof.vals()) {
      switch (hexTextToBlob(proof)) {
        ...
        case (#err(msg)) { return #err("Invalid storage proof element: " # msg) };
      };
    };
